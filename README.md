...
2. ****Create a topic****:
    ```bash
    docker exec kafka-container kafka-topics --create --bootstrap-server localhost:9092 --replication-factor 3 --partitions 4 --topic topicname
    ```
3. **List topics**:
    ```bash
...
4. **Produce messages to the topic**:
    ```bash
    docker exec -it kafka-container kafka-console-producer --bootstrap-server localhost:9092 --topic rushi
    ```
